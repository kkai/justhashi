...
nonisolated enum StoreProduct {
    static let fullUnlock = "de.kaikunze.hashi.full"
}
...
final class EntitlementStore {
    ...
    private enum Key {
        ...
        ///
        /// This is also what makes `HomeView`'s picker restoration safe — see
        /// the note there about the cold-launch race.
        ///
        ...
        /// validation backend.
        static let unlocked = "hashi.entitlement.v1"
    }
    ...
    func restore() async {
        ...
        do {
            ...
            purchaseState = isUnlocked
                ? .idle
                : .note("Nothing to restore on this Apple Account. If you bought Just Hashi with a "
...
struct StoreKitEntitlementSource: EntitlementSource {
    ...
    ///
    /// Just Hashi ships iPhone and iPad only, so the visionOS branch never
    /// compiles here. It is kept for the same reason `PlatformSeams` keeps its
    /// tvOS seams: it costs nothing, and the lesson has already been paid for.
    ...
    /// an app's windows open at once and the system has to know which one the
    /// sheet belongs to.
