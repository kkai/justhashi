...
/// The edge swipe used to be able to pop a puzzle mid-drag. These exercise the
...
struct SwipeBackTests {
    ...
    /// Every screen the player drags across has to carry the modifier, or the
    /// bug comes back on whichever one was missed.
    @Test func everyBoardScreenDisablesIt() throws {
        let root = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent().deletingLastPathComponent()
        for (file, screens) in [("Hashi/Game/GameView.swift", 1),
                                ("Hashi/Teaching/TeachingViews.swift", 2)] {
