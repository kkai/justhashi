...
/// The purchase and restore surface is where silent no-ops hide: a cancelled
/// sheet, a parent-approval request and a verification failure all look
/// identical from the outside. The seam returns an outcome rather than a Bool
...
@Suite struct PurchaseFlowTests {
    ...
    private func scratch(_ name: String) -> UserDefaults {
        let suite = "hashi.purchase.\(name).\(UUID().uuidString)"
    ...
    /// Ask to Buy must not be indistinguishable from a cancelled sheet — that
    /// would put the buy button back and say nothing at all.
    ...
    /// The player may have been charged. Saying nothing is the worst response.
    ...
    /// The state lives on an app-lifetime object, so an error raised on one
    /// screen would otherwise greet the player on another.
